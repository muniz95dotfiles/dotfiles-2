...
dunstify "$volume""      " -i "$icon_name" -t 2000 -h int:value:"$volume" -h string:synchronous:"─" -r 555
    else
...
case $1 in
	...
	if is_mute ; then
    DIR=`dirname "$0"`
    dunstify -i "/usr/share/icons/Faba/48x48/notifications/notification-audio-volume-muted.svg" -r 555 -u normal "Mute" -t 2000
	else
	    send_notification
	fi
	;;
